...
describe("exact registry release verification", () => {
  it("requires every exact integrity and the requested tag", () => {
    const readState = vi.fn((name: string) => ({ name, version: "1.0.0", integrity, tags: { next: "1.0.0" } }));
    expect(verifyRegistryReleaseSet({ releaseSet, tag: "next", readState })).toHaveLength(RELEASE_PACKAGE_NAMES.length);
    expect(readState).toHaveBeenCalledTimes(RELEASE_PACKAGE_NAMES.length);
    expect(() => verifyRegistryReleaseSet({ releaseSet: { packages: [] }, tag: "next", readState })).toThrow(/exact public-package/u);
