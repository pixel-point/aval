...
const installSources=(sources,suffix="")=>{const elements=sources.map(source=>{const element=document.createElement("source");element.setAttribute("src",source.src.split("#")[0]+suffix+(suffix?"":"#v="+String(build.generation)));element.setAttribute("type",source.type);element.setAttribute("integrity",source.integrity);return element;});motion.replaceChildren(...elements);};
