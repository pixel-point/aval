...
export const DEFAULT_VIDEO_CANDIDATE_CLOCK: PathSchedulerClock = Object.freeze({
  now: (): number => typeof performance === "undefined"
    ? Date.now()
    : performance.now()
});

export const DEFAULT_VIDEO_CANDIDATE_TIMERS: VideoCandidateTimerHost =
  Object.freeze({
...
export class VideoCandidateOperationControl {
  ...
  #expire(): void {
    this.#abort(new DOMException(
      "video candidate preparation deadline expired",
      "TimeoutError"
    ));
  }
...
export async function raceVideoCandidateOperation<T>(
...
export function validateVideoCandidateFactoryOptions(
  ...
  if (
    ...
    throw new TypeError("video candidate readiness factory is malformed");
  }
  ...
  if (
    ...
    throw new TypeError("video candidate timer host is malformed");
  }
  if (
    options.prepareCache !== undefined &&
    typeof options.prepareCache !== "function"
  ) {
    throw new TypeError("video candidate cache preparer must be a function");
  }
  if (
    ...
    throw new TypeError("video candidate resource host is malformed");
  }
  if (
...
export function abortReason(signal: AbortSignal): DOMException {
  return signal.reason instanceof DOMException
    ? signal.reason
    : new DOMException("video candidate operation aborted", "AbortError");
}
...
function validateFactory(
  ...
  if (
    ...
    throw new TypeError(`video candidate ${label} factory is malformed`);
  }
}
...
function validatePlaybackSession(session: IntegratedPlaybackSession): void {
  if (
    ...
    throw new TypeError("prepared video playback session is malformed");
  }
}

function validateDeadline(value: number): void {
  if (!Number.isFinite(value) || value < 0 || value > Number.MAX_SAFE_INTEGER) {
    throw new RangeError("video candidate deadline must be finite and safe");
  }
}
...
function checkedNow(clock: PathSchedulerClock): number {
  ...
  if (!Number.isFinite(value) || value < 0) {
    throw new RangeError("video candidate clock must be finite and non-negative");
  }
  return value;
}
