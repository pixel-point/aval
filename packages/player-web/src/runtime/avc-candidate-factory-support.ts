...
export const DEFAULT_AVC_CANDIDATE_CLOCK: PathSchedulerClock = Object.freeze({
  now: (): number => typeof performance === "undefined"
    ? Date.now()
    : performance.now()
});

export const DEFAULT_AVC_CANDIDATE_TIMERS: AvcCandidateTimerHost =
  Object.freeze({
...
export class AvcCandidateOperationControl {
  ...
  #expire(): void {
    this.#abort(new DOMException(
      "AVC candidate preparation deadline expired",
      "TimeoutError"
    ));
  }
...
export async function raceAvcCandidateOperation<T>(
...
export function validateAvcCandidateFactoryOptions(
...
export const validateOpaqueCandidateFactoryOptions =
  validateAvcCandidateFactoryOptions;
