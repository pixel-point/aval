...
import type { BrowserVideoCandidateHub } from "./browser-video-candidate-hub.js";
...
export interface BrowserReadinessTick {
...
export interface BrowserReadinessRouteRun {
  readonly ticks: readonly Readonly<BrowserReadinessTick>[];
...
export interface BrowserReadinessCleanup {
 ...
 */
export class BrowserReadinessDriver {
  ...
  readonly #session: BrowserVideoPlaybackSession;
  readonly #tickLimit: number;
  #nextOrdinal = 1n;
  #lastTick: Readonly<BrowserReadinessTick> | null = null;
  #disposed = false;
  ...
  public static async create(options: {
  ...
  }): Promise<BrowserReadinessDriver> {
    assertReadinessActive(options.candidate);
    ...
    let session: BrowserVideoPlaybackSession | null = null;
    try {
      session = await BrowserVideoPlaybackSession.create({
      ...
      const driver = new BrowserReadinessDriver({
        candidate: options.candidate,
        hub: options.hub,
        graph,
        session,
        tickLimit: readinessTickLimit(options.candidate)
      });
  ...
  public get lastTick(): Readonly<BrowserReadinessTick> | null {
  ...
  public playbackSnapshot(): ReturnType<BrowserVideoPlaybackSession["snapshot"]> {
  ...
  } = {}): Promise<Readonly<BrowserReadinessTick>> {
    this.#assertUsable();
    assertReadinessActive(this.#candidate);
  ...
  public async driveEdge(
    ...
    options: { readonly leaveTargetUnsettled?: boolean } = {}
  ): Promise<Readonly<BrowserReadinessRouteRun>> {
    this.#assertUsable();
  ...
  public async dispose(): Promise<Readonly<BrowserReadinessCleanup>> {
    if (!this.#disposed) {
      this.#disposed = true;
...
export function assertReadinessActive(
  input: Readonly<Pick<
    VideoCandidateReadinessSessionInput,
