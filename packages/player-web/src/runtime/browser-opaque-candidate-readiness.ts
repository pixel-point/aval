...
export class BrowserOpaqueCandidateReadinessFactory
  implements OpaqueCandidateReadinessFactory {
  ...
  public create(
    ...
    return new BrowserOpaqueReadinessSession(input, this.#hub, this.#now);
  }
}

class BrowserOpaqueReadinessSession
  implements OpaqueCandidateReadinessSession {
  ...
  #prepared: BrowserOpaquePlaybackSession | null = null;
  #disposed = false;
  ...
  public async prepareActivation(
    ...
    this.#assertActive();
    const playback = await BrowserOpaquePlaybackSession.create({
      candidate: this.#input,
      activation: input,
      hub: this.#hub
    });
    this.#prepared = playback;
    this.#hub.activate(
...
function requireCutRunway(
  edge: string,
  plan: Readonly<OpaqueCandidateReadinessSessionInput["interactionCache"]>
) {
...
function remainingMs(input: Readonly<OpaqueCandidateReadinessSessionInput>): number {
